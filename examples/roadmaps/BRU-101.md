...
| Verb        | Command                                                                                   |
| ----------- | ----------------------------------------------------------------------------------------- |
| `branch`    | `git fetch origin && git switch -c <type>/<KEY>-<slug> origin/integration/BRU-101`         |
| `push`      | `git push -u origin <branch>`, then confirm `git rev-parse HEAD` == `git rev-parse @{u}`.  |
| `open-pr`   | `gh pr create --base integration/BRU-101 --title "<type>(<KEY>): ..." --body-file pr.md`   |
| `comment-pr`| `gh pr comment <N> --body-file review.md`                                                  |
| `merge`     | `gh pr merge <N> --squash --delete-branch`                                                  |
| `reconcile` | `git switch main && git pull`, `git merge --no-ff integration/BRU-101`, run `verify`, then `git push origin main`. Once, at mission-complete. |
-->
