...
You materialize the project config from Autopilot's packaged examples. In the Codex adapter,
the install instructions copy those examples into an adapter support directory next to the
installed skills — not into the consuming project's source tree.
...
- Preferred path: resolve this skill's own directory, then read templates from
  ...
  `.agents/autopilot/examples/`; with the documented global install, that is
  `~/.agents/autopilot/examples/`.
